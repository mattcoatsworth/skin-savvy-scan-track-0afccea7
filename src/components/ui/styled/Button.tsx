...
export interface ButtonProps
  ...
  variant?: 'default' | 'destructive' | 'outline' | 'secondary' | 'ghost' | 'link' | 'skinGoals';
  size?: 'default' | 'sm' | 'lg' | 'icon';
  asChild?: boolean;
}

/**
 * Button component using our JavaScript styles system
...
export const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(({
  ...
  // Map our JS style system to Tailwind classes (we'll keep using Tailwind for now)
  // Later this can be converted to React Native StyleSheet
  ...
  const variantClasses = {
    ...
    ghost: "hover:bg-[hsl(var(--ghost-hover))] hover:text-accent-foreground",
    link: "text-primary underline-offset-4 hover:underline",
    skinGoals: "text-white rounded-md py-3 px-4 w-full text-center font-medium skin-goals-button"
  };
