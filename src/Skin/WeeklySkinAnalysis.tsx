...
import { AIAnalysis, WeeklySkinData, DaySkinData } from "./types";
import { getDifficultyBadgeClass, getCategoryBadgeClass } from "./utils";

const WeeklySkinAnalysis: React.FC = () => {
  useScrollToTop();
  
  // Sample data for weekly skin health
  const weeklySkinData: WeeklySkinData = {
    ...
    <div className="pb-20">
            ...
            <div className="space-y-4">
              <div>
                <div className="flex justify-between items-end mb-1">
                  <span className="text-3xl font-bold">{weeklySkinData.overallScore}</span>
                  <span className="text-green-600 text-sm">+{weeklySkinData.overallScore - (weeklySkinData.previousWeekScore || 0)} from last week</span>
                </div>
                <Progress value={weeklySkinData.overallScore} className="h-2" />
              </div>
              
              <div>
                <p className="text-sm text-muted-foreground mb-3">Weekly Trend</p>
                <TrendChart data={weeklySkinData.weeklyTrend} height={100} />
              </div>
            </div>
          </CardContent>
        </Card>
      ...
      <div className="mb-6">
        ...
        <div className="space-y-3">
          {weeklySkinData.dailyScores.map((day, index) => (
            <Card key={index}>
              <CardContent className="p-4">
      ...
      <div className="mb-6">
        ...
        <Card className="mb-4">
          <CardContent className="p-4 space-y-3">
            {weeklySkinData.skinParameters.map((param, index) => (
              <div key={index}>
        ...
        <div className="space-y-4">
          ...
          <Card>
            <CardContent className="p-4">
              ...
              <ul className="space-y-2">
                {weeklySkinData.positiveFactors.map((factor, index) => (
          ...
          <Card>
            <CardContent className="p-4">
              ...
              <ul className="space-y-2">
                {weeklySkinData.negativeFactors.map((factor, index) => (
                  <li key={index} className="flex justify-between text-sm">
                    <span>{factor.name}</span>
